...
class content_contenttypesapi_pagesetter_publistPlugin extends contentTypeBase
{
  ...
  function getName() { return 'pagesetter_publist'; }
  function getTitle() { return _CONTENT_CONTENTENTTYPE_PAGESETTER_PUBLISTTITLE; }
  function getDescription() { return _CONTENT_CONTENTENTTYPE_PAGESETTER_PUBLISTDESCR; }
  ...
  function display()
  {
    ...
    if (is_array($filters) && strlen(trim($filters[0])))	$plargs['filterSet'] = $filters;
    ...
    if ($publist !== false) {
      ...
      {
        $pub = pnModAPIFunc('pagesetter', 'user', 'getPubFormatted',
                            array('tid'            => $this->tid,
                                  'pid'            => $pub['pid'],
                                  'format'         => $this->tpl,
                                  'updateHitCount' => false));
        if ($pub !== false)  $publications[] = $pub;
      }
    }
