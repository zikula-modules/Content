...
class contentTypeBase
{
   ...
   * @return string
   */
  function getModule() { return 'unknown'; }
   ...
   * @return string
   */
  function getName() { return 'unknown'; }
   ...
   * @return string
   */
  function getTitle() { return '- no title defined -'; }
   ...
   * @return bool True
   */
  function loadData(&$data) { return true; }
  ...
  */
   ...
   * @return string
   */
  function display() { return '- no display function defined -'; }
  ...
  */
   ...
   * @return string
   */
  function displayEditing() { return $this->getTitle(); }
   ...
   * @return array
   */
  function getDefaultData() { return null; }
   ...
   */
  function getSearchableText() { return null; }
};
...
function content_contentapi_newContent($args)
{
  ...
  if ($addVersion)
  {
    $ok = pnModAPIFunc('content', 'history', 'addPageVersion',
                       array('pageId' => $pageId,
                             'action'  => '_CONTENT_HISTORYCONTENTADDED' /* delayed translation */));
    if ($ok === false)
      return false;
  }
...
function content_contentapi_updateContent($args)
{
  ...
  {
    $ok = pnModAPIFunc('content', 'history', 'addPageVersion',
                       array('pageId' => $content['pageId'],
                             'action'  => '_CONTENT_HISTORYCONTENTUPDATED' /* delayed translation */));
    if ($ok === false)
      return false;
  }
...
function content_contentapi_deleteContent($args)
{
  ...
  if ($addVersion)
  {
    $ok = pnModAPIFunc('content', 'history', 'addPageVersion',
                       array('pageId' => $content['pageId'],
                             'action'  => '_CONTENT_HISTORYCONTENTDELETED' /* delayed translation */));
    if ($ok === false)
      return false;
  }
...
function content_contentapi_deletePageAndSubPageContent($args)
{
  ...
  $contentItems = pnModAPIFunc('content', 'content', 'getPageAndSubPageContent',
                              array('pageId' => $pageId));
...
function content_contentapi_getTranslationInfo($args)
{
  ...
  if ($contentId != null)
  {
    $contentItem = pnModAPIFunc('content', 'content', 'getContent', 
                                array('id' => $contentId));
  ...
  foreach ($translatableItems as $item)
  {
    if ($item['plugin']->isTranslatable())
    {
      $translationItems[] = array('text' => $layout->getContentAreaTitle($item['areaIndex']) . ": $item[type] ($i/$count)",
                                  'value' => $item['id']);
  ...
  return array('items' => $translationItems,
...
function content_contentapi_dragContent($args)
{
  ...
  $ok = pnModAPIFunc('content', 'history', 'addPageVersion',
                     array('pageId' => $pageId,
                           'action'  => '_CONTENT_HISTORYCONTENTMOVED' /* delayed translation */));
  if ($ok === false)
    return false;
...
function contentInsertContent($contentId, $position, $contentAreaIndex, $pageId)
{
  ...
  $contentData = array('id' => $contentId, 
                       'position' => $position,
                       'areaIndex' => $contentAreaIndex);
  DBUtil::updateObject($contentData, 'content_content');
...
function content_contentapi_getContentTypes($args)
{
  ...
  for ($i=0,$cou=count($plugins); $i<$cou; ++$i)
  {
    ...
    {
      $contentTypes[] = array('module' => $plugin->getModule(),
                              'name' => $plugin->getName(),
