    ...
    <table class="z-datatable">
        <thead>
            <tr>
                ...
                {if $categoryUsage lt 4}<th>{gt text="Category"}</th>{/if}
        ...
        <tbody>
            ...
            <tr class="{cycle values="z-odd,z-even"}">
                ...
                {if $categoryUsage lt 4}<td>{foreach from=$categories[$page.id] item=cat name=categories}{$cat}{if !$smarty.foreach.categories.last}<br />{/if}{/foreach}</td>{/if}
