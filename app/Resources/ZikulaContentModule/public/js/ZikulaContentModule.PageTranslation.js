...
function contentPageInitYandexSupport(yandexApiKey) {
    jQuery('#contentTranslateTarget .tab-pane .form-control').each(function (index) {
        ...
        parent.find('.input-group').append('<div class="input-group-append"><button class="btn btn-default add-suggestion" type="button" title="' + Translator.trans('Insert suggestion for translation', {}, 'page') + '"><i class="fas fa-book"></i></button></div></div>');
    });
