...
function contentInitTocEdit() {
    ...
    contentTocChangedSelection = function() {
        jQuery('#' + fieldPrefix + 'includeHeadingLevel').parents('.form-group').toggleClass('d-none', parseInt(jQuery('#' + fieldPrefix + 'includeHeading').val()) < 2);
        jQuery('#' + fieldPrefix + 'includeSubpageLevel').parents('.form-group').toggleClass('d-none', parseInt(jQuery('#' + fieldPrefix + 'includeSubpage').val()) < 2);
    };
