...
class contentLayoutBase
{
  ...
  function getTitle() { return 'Unknown layout'; }
  function getDescription() { return 'This is the base class for layouts - do not use!'; }
  function getNumberOfContentAreas() { return 0; }
  function getContentAreaTitle($areaIndex) { return $areaIndex; }
}
...
function content_layoutapi_getLayouts($args)
{
  ...
  {
    $plugin = &$plugins[$i];
    $layouts[] = array('name' => $plugin->getName(),
...
function content_layoutapi_getLayout($args)
{
  ...
  return array('name' => $plugin->getName(),
               'title' => $plugin->getTitle(),
               'description' => $plugin->getDescription(),
               'numberOfContentAreas' => $plugin->getNumberOfContentAreas());
}
