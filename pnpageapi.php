...
$orderBy";
  ...
  for ($i=0,$cou=count($pages); $i<$cou; ++$i)
  {
    ...
    if ($includeContent)
    {
      $content = pnModAPIFunc('content', 'content', 'getPageContent', 
                              array('pageId' => $p['id'],
                                    'editing' => $editing,
                                    'translate' => $translate));
      if ($content === false)
...
function contentGetPageListRestrictions($filter, &$restrictions, &$join)
{
  ...
  {
    $pageData = pnModAPIFunc('content', 'page', 'getPage',
                                array('id' => $filter['superParentId']));
    if ($pageData === false)
      return false;
...
function content_pageapi_newPage($args)
{
  ...
  if ($pageId > 0)
  {
    $sourcePageData = pnModAPIFunc('content', 'page', 'getPage', 
                                   array('id' => $pageId,
                                         'enableEscape' => false,
  ...
  $ok = pnModAPIFunc('content', 'page', 'isUniqueUrlnameByParentID', 
                     array('urlname' => $pageData['urlname'], 
                           'parentId' => $pageData['parentPageId']));
  if (!$ok)
    return LogUtil::registerError(_CONTENT_NOTUNIQUEURLNAME);
  ...
  $ok = content_pageapi_insertPage(array('pageId' => $pageData['id'],
                                         'position' => $pageData['position'],
                                         'parentPageId' => $pageData['parentPageId']));
  if ($ok === false)
    return false;

  $ok = pnModAPIFunc('content', 'history', 'addPageVersion',
                     array('pageId' => $pageData['id'],
...
function content_pageapi_updatePage($args)
{
  ...
  $oldPageData = pnModAPIFunc('content', 'page', 'getPage', 
                              array('id' => $pageId,
                                    'editing' => true,
...
function content_pageapi_updateTranslation($args)
{
  ...
  // Insert new
  $translatedData = array('pageId' => $pageId, 
                          'language' => $language,
  ...
  if ($addVersion)
  {
    $ok = pnModAPIFunc('content', 'history', 'addPageVersion',
                       array('pageId' => $pageId,
                             'action'  => '_CONTENT_HISTORYTRANSLATED' /* delayed translation */));
    if ($ok === false)
      return false;
  }
...
function content_pageapi_deleteTranslation($args)
{
  ...
  $ok = pnModAPIFunc('content', 'content', 'deletePageTranslations',
                     array('pageId' => $pageId,
                           'language' => $language));
  if ($ok === false)
    return false;
  ...
  if ($addVersion)
  {
    $ok = pnModAPIFunc('content', 'history', 'addPageVersion',
                       array('pageId' => $pageId,
                             'action'  => '_CONTENT_HISTORYTRANSLATIONDEL' /* delayed translation */));
    if ($ok === false)
      return false;
  }
...
function content_pageapi_deletePage($args)
{
  ...
  $ok = pnModAPIFunc('content', 'content', 'deletePageAndSubPageContent',
                     array('pageId' => $pageId));
  if (!$ok)
    return false;
...
WHERE     $pageColumn[setLeft] > $pageData[setLeft]
  ...
  $ok = pnModAPIFunc('content', 'history', 'deletePage',
                     array('pageId' => $pageData['id']));
  if ($ok === false)
    return false;
...
function content_pageapi_drag($args)
{
  ...
  if ($srcPage['setLeft'] < $dstPage['setLeft'] && $srcPage['setRight'] > $dstPage['setRight'])
    return LogUtil::registerError(_CONTENT_CIRCULARREFERENCEERROR);
  ...
  $test = pnModAPIFunc('content', 'page', 'isUniqueUrlnameByParentID', 
                       array('urlname' => $srcPage['urlname'], 
                             'parentId' => $dstPage['parentPageId'],
                             'currentPageId' => $srcId));
  if (!$test) {
        pnModAPIFunc('content', 'page', 'insertPage', 
                     array('pageId' => $srcId,
                           'position' => $srcPage['position'],
                           'parentPageId' => $srcPage['parentPageId']));
        // FIXME: This causes a "page not found". But I don't know why. Pls help ;)
        return LogUtil::registerError(_CONTENT_NOTUNIQUEURLNAME);
  }
  $ok = pnModAPIFunc('content', 'page', 'insertPage', 
                 array('pageId' => $srcId,
                       'position' => $dstPage['position']+1,
                       'parentPageId' => $dstPage['parentPageId']));
  if ($ok === false)
    return false;
...
WHERE $pageData[setLeft] <= $pageColumn[setLeft] AND $pageColumn[setRight] <= $pageData[setRight]";
  ...
  // Update this page
  $newPageData = array('id' => $pageId, 
                       'position' => $position,
                       'parentPageId' => $parentPageId,
                       'level' => ($parentPageData == null ? 0 : $parentPageData['level']+1));
