...
class content_edit_mainHandler extends pnFormHandler
{
  ...
  function initialize(&$render)
  {
    ...
    $pages = pnModAPIFunc('content', 'page', 'getPages', 
                          array('editing' => true,
    ...
    PageUtil::setVar('title', _CONTENT_MAINHEADER);
  ...
  function handleCommand(&$render, &$args)
  {
    ...
    else if ($args['commandName'] == 'drag')
    {
      ...
      $ok = pnModAPIFunc('content', 'page', 'drag',
                         array('srcId' => $srcId,
                               'dstId' => $dstId));
      if (!$ok)
        return $render->pnFormRegisterError(null);
    ...
    else if ($args['commandName'] == 'incIndent')
    {
      ...
      $ok = pnModAPIFunc('content', 'page', 'increaseIndent',
                         array('pageId' => $pageId));
      if (!$ok)
        return $render->pnFormRegisterError(null);
    }
    else if ($args['commandName'] == 'deletePage')
    {
      ...
      $ok = pnModAPIFunc('content', 'page', 'deletePage',
                         array('pageId' => $pageId));
      if ($ok === false)
        return $render->pnFormRegisterError(null);
    }
...
class content_edit_newPageHandler extends pnFormHandler
{
  ...
  function initialize(&$render)
  {
    ...
    PageUtil::setVar('title', _CONTENT_NEWPAGEHEADER);

    $render->assign('layouts', $layouts);
    $render->assign('page', $page);
    $render->assign('location', $this->location);
    if ($this->location == 'sub')
      $render->assign('locationLabel', _CONTENT_LOCATIONSUB);
    else
      $render->assign('locationLabel', _CONTENT_LOCATION);
  ...
  function handleCommand(&$render, &$args)
  {
    ...
    if ($args['commandName'] == 'create')
    {
      ...
      $id = pnModAPIFunc('content', 'page', 'newPage',
                         array('page' => $pageData,
                               'pageId' => $this->pageId,
                               'location' => $this->location));
      if ($id === false)
        return false; //LogUtil::registerStatus(_CONTENT_ADMINUPDATED);
...
class content_edit_editPageHandler extends pnFormHandler
{
  ...
  function initialize(&$render)
  {
    ...
    pnModAPIFunc('PageLock', 'user', 'pageLock',
                 array('lockName' => "contentPage{$this->pageId}",
                       'returnUrl' => $returnUrl));
  ...
  function handleCommand(&$render, &$args)
  {
    if ($args['commandName'] == 'create')
    {
      ...
      $id = pnModAPIFunc('content', 'content', 'newContent',
                         array('content' => $contentData,
                               'pageId' => $this->pageId,
                               'contentAreaIndex' => $this->contentAreaIndex,
...
class content_edit_editContentHandler extends pnFormHandler
{
  ...
  function initialize(&$render)
  {
    ...
    pnModAPIFunc('PageLock', 'user', 'pageLock',
                 array('lockName' => "contentContent{$this->contentId}",
                       'returnUrl' => $returnUrl));
  ...
  function handleCommand(&$render, &$args)
  {
    ...
    if ($args['commandName'] == 'save' || $args['commandName'] == 'translate')
    {
      ...
      $ok = pnModAPIFunc('content', 'content', 'updateContent',
                         array('content' => $contentData + $contentData['content'],
...
class content_edit_translatePageHandler extends pnFormHandler
{
  ...
  function initialize(&$render)
  {
    ...
    if ($this->language == $page['language'])
      return $render->pnFormRegisterError(LogUtil::registerError(_CONTENT_CANNOTTANSLATEDEFAULTLANGUAGE));
...
class content_edit_translateContentHandler extends pnFormHandler
{
  ...
  function initialize(&$render)
  {
    ...
    if ($this->backref != null)
      ...
      $returnUrl = pnModUrl('content', 'edit', 'editpage', array('pid' => $this->pageId));
    pnModAPIFunc('PageLock', 'user', 'pageLock',
                 array('lockName' => "contentTranslateContent{$this->contentId}",
                       'returnUrl' => $returnUrl));
  ...
  function handleCommand(&$render, &$args)
  {
    ...
    if ($args['commandName'] == 'next' || $args['commandName'] == 'prev'
        || $args['commandName'] == 'quit' || $args['commandName'] == null /* Auto postback */)
    {
      if (!$render->pnFormIsValid())
        return false;
      ...
      $ok = pnModAPIFunc('content', 'content', 'updateTranslation',
                         array('translated' => $contentData['translated'],
    ...
    {
      $ok = pnModAPIFunc('content', 'content', 'deleteTranslation',
                         array('contentId' => $this->contentId,
                               'language' => $this->language));
      if ($ok === false)
        return $render->pnFormRegisterError(null);
...
class content_edit_historyContentHandler extends pnFormHandler
{
  ...
  function initialize(&$render)
  {
    ...
    $versions = pnModAPIFunc('content', 'history', 'getPageVersions',
                             array('pageId' => $this->pageId));
    if ($versions === false)
      return $render->pnFormRegisterError(null);
  ...
  function handleCommand(&$render, &$args)
  {
    ...
    {
      $ok = pnModAPIFunc('content', 'history', 'restoreVersion',
                         array('id' => $args['commandArgument']));
      if ($ok === false)
        return $render->pnFormRegisterError(null);
    }
